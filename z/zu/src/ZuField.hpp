...
#define ZuField_Decl_(U, Accessor, ...) \
  ZuPP_Defer(ZuField_Decl__)(U, ZuPP_Strip(Accessor), (__VA_ARGS__))
#define ZuField_Decl(U, Args) ZuPP_Defer(ZuField_Decl_)(U, ZuPP_Strip(Args))

#define ZuField_Type__(U, ID, ...) ZuFieldType(U, ID)
#define ZuField_Type_(U, Accessor, ...) \
  ZuPP_Defer(ZuField_Type__)(U, ZuPP_Strip(Accessor))
...
class ZuFieldTuple__ : public ZuTuple<Map<typename Fields::T>...> {
  using Base = ZuTuple<Map<typename Fields::T>...>;
public:
  ZuFieldTuple__(const ZuFieldTuple__ &) = default;
  ZuFieldTuple__ &operator =(const ZuFieldTuple__ &) = default;
  ZuFieldTuple__(ZuFieldTuple__ &&) = default;
  ZuFieldTuple__ &operator =(ZuFieldTuple__ &&) = default;
  ZuFieldTuple__(const Base &v) : Base{v} { }
  ZuFieldTuple__ &operator =(const Base &v) {
    Base::operator =(v);
    return *this;
  }
  ZuFieldTuple__(Base &&v) : Base{ZuMv(v)} { }
  ZuFieldTuple__ &operator =(Base &&v) {
    Base::operator =(ZuMv(v));
    return *this;
  }
  ZuFieldTuple__(const V &v) : Base{Fields::get(&v)...} { }
  ZuFieldTuple__ &operator =(const V &v) {
    ~Base();
...
template <typename V, template <typename> typename Map, typename ...Fields>
struct ZuFieldTuple__<V, Map, ZuTypeList<Fields...>> :
  public ZuFieldTuple__<V, Map, Fields...> { };

template <typename V, typename ...Fields>
struct ZuFieldTuple_ {
  static decltype(auto) get(const V &v) {
    return ZuFieldTuple__<V, ZuCRef, Fields...>{v};
  }
  static decltype(auto) get(V &v) {
    return ZuFieldTuple__<V, ZuLRef, Fields...>{v};
  }
  static decltype(auto) get(V &&v) {
    return ZuFieldTuple__<V, ZuAsIs, Fields...>{ZuMv(v)};
  }
};
...
struct ZuFieldTuple {
  ...
  static decltype(auto) get(P &&v) {
    return ZuFieldTuple_<ZuDecay<P>, Fields...>::get(ZuFwd<P>(v));
  }
};

// key/value extraction given key # (0 is default for primary key)
template <typename V, unsigned KeyID = 0>
struct ZuFieldAccessor {
  template <typename T>
