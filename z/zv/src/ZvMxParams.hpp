  ...
  void init(ZuString id, const ZvCf *cf) {
	...
	threadsCf->all([&sched](ZvCfNode *node) {
	  if (auto threadCf = node->cf) {
	    ...
	    thread.detached(
		threadCf->getInt("detached", 0, 1, thread.detached()));
	  }
	});
      }
    }
    ...
    trace(cf->getInt("trace", 0, 1, trace()));
    debug(cf->getInt("debug", 0, 1, debug()));
    frag(cf->getInt("frag", 0, 1, frag()));
    yield(cf->getInt("yield", 0, 1, yield()));
#endif
  }
};
