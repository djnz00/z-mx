...
class ZiVRing_Breakpoint {
public:
  ZiVRing_Breakpoint() : m_enabled(false), m_oneshot(false) { }
...
#define ZiVRing_FUNCTEST
#include <zlib/ZiVRing.hpp>
#include "../src/ZiVRing.cpp"
...
public:
  ...
  void start(unsigned nThreads, ZiVRingParams params);
  void stop();
...
void App::start(unsigned nThreads, ZiVRingParams params)
{
...
void asynchronous_(int tid, Work *work, ZiVRing_Breakpoint &bp)
{
  bp.enable();
...
{
#ifndef _WIN32
  unlink("/dev/shm/ZiVRingTest.ctrl");
  unlink("/dev/shm/ZiVRingTest.data");
#endif
}
