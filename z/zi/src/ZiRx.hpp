...
public:
  ...
  void recv(ZiIOContext &io) {
    ...
    io.init(ZiIOFn{ZuMv(buf), [](Buf *buf, ZiIOContext &io) -> uintptr_t {
      ...
      // scan header
      int frameLen = ZuInvoke<Hdr>(impl(), io, buf);
	...
	return 0;
      ...
      // process body
      frameLen = ZuInvoke<Body>(impl(), io, buf, frameLen);
  ...
  void recvAsync(ZiIOContext &io) {
	...
	buf->length = frameLen;
      ...
      // process body
      frameLen = ZuInvoke<Body>(impl(), io, io.fn.mvObject<Buf>());
      if (ZuUnlikely(frameLen < 0)) return 0;
      if (!frameLen) return len;
  ...
  int recvMem(const uint8_t *data, unsigned rxLen, ZmRef<Buf> &buf) {
    ...
    while (len) {
      // scan header
      int frameLen = ZuInvoke<Hdr>(impl(), buf.ptr());
      if (ZuUnlikely(frameLen < 0)) return -1;
	...
	return 0;
      ...
      // process body
      frameLen = ZuInvoke<Body>(impl(), buf.ptr(), frameLen);
  ...
  int recvMemAsync(const uint8_t *data, unsigned rxLen, ZmRef<Buf> &buf) {
	...
	buf->length = frameLen;
      ...
      // process body
      frameLen = ZuInvoke<Body>(impl(), ZuMv(buf));
      if (ZuUnlikely(frameLen < 0)) return -1; // error
      if (!frameLen) return rxLen; // EOF - discard remainder
