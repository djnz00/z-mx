...
void ZiIPC::getpinfo(uint32_t &pid, ZmTime &start)
{
#ifdef linux
  pid = getpid();
...
bool ZiIPC::alive(uint32_t pid, ZmTime start)
{
  if (!pid) return false;
#ifdef linux
  ZuStringN<20> path; path << "/proc/" << ZuBox<uint32_t>(pid);
...
bool ZiIPC::kill(uint32_t pid, bool coredump)
{
  if (!pid) return false;
#ifdef linux
...
int ZiIPC::open_(ZeError *e) { return Zi::OK; }

int ZiIPC::close_(ZeError *e) { return Zi::OK; }

int ZiIPC::wait(ZmAtomic<uint32_t> &addr, uint32_t val)
{
...
int ZiIPC::wake(ZmAtomic<uint32_t> &addr, int n)
{
  addr &= ~Waiting;
  syscall(SYS_futex, reinterpret_cast<volatile int *>(&addr),
      FUTEX_WAKE, n, 0, 0, 0);
...
int ZiIPC::wait(unsigned index, ZmAtomic<uint32_t> &addr, uint32_t val)
{
  if (addr.cmpXch(val | Waiting, val) != val) return Zi::OK;
  val |= Waiting;
...
int ZiIPC::wake(unsigned index, ZmAtomic<uint32_t> &addr, int n)
{
  addr &= ~Waiting;
