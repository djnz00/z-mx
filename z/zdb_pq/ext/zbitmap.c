...
Datum zbitmap_or(PG_FUNCTION_ARGS) {
  zu_bitmap *v = (zu_bitmap *)VARDATA(PG_GETARG_POINTER(0));
  const zu_bitmap *p = (const zu_bitmap *)VARDATA(PG_GETARG_POINTER(1));
...
Datum zbitmap_lt(PG_FUNCTION_ARGS) {
  const zu_bitmap *l = (const zu_bitmap *)VARDATA(PG_GETARG_POINTER(0));
  const zu_bitmap *r = (const zu_bitmap *)VARDATA(PG_GETARG_POINTER(1));
