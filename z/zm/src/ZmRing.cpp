    ...
    const Params &params)
{
  /* new (&logBuf[(logIndex++) & 1023])
    Log{"wait", ZmTimeNow(), 0, this, reinterpret_cast<uint32_t *>(&addr), val}; */
  if (ZuUnlikely(params.timeout)) {
	...
	if (syscall(SYS_futex, reinterpret_cast<volatile int *>(&addr),
	  ...
	  auto errno_ = errno;
	  /* new (&logBuf[(logIndex++) & 1023])
	    Log{"woke", ZmTimeNow(), errno_, this, reinterpret_cast<uint32_t *>(&addr), addr}; */
	...
	/* new (&logBuf[(logIndex++) & 1023])
	  Log{"woke", ZmTimeNow(), errno, this, reinterpret_cast<uint32_t *>(&addr), addr}; */
	i = 0;
      } else
	++i;
    } while (addr == val);
...
{
  /* new (&logBuf[(logIndex++) & 1023])
    Log{"wake", ZmTimeNow(), 0, this, reinterpret_cast<uint32_t *>(&addr), addr.load_()}; */
  syscall(SYS_futex, reinterpret_cast<volatile int *>(&addr),
